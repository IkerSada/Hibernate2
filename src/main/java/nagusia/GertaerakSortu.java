...
public class GertaerakSortu {
	...
	private List<LoginGertaera> gertaerakZerrendatu() {
		...
		try {
			em.getTransaction().begin();
			List<LoginGertaera> result = em.createQuery("from LoginGertaera",
					LoginGertaera.class).getResultList();
			em.getTransaction().commit();
			return result;
		} catch (Exception e) {
			if (em.getTransaction().isActive()) {
	...
	public static void main(String[] args) {
		...
		// }
		    ...
		    try {
		        ...
		        u.setPasahitza("1236");
		        e.printObjMemDB("u.setPasahitza(\"1236\") exekutatu da, baina objektua ez dago DBarekin konektatuta (detached)", u);
		    } catch (Exception ex) {
		        if (em.getTransaction().isActive()) {
		            em.getTransaction().rollback();
		        }
		    ...
		    try {
		        ...
		        System.out.println("save => ERROREA: objektua 'detached' dagoenez, 'save' horrekin objektu berdina berriz ere sartzen saiatzen da, eta gako-errorea sortuko da");
		        if (em.getTransaction().isActive()) {
		            em.getTransaction().rollback();
		        }
		    } finally {
		    ...
		    try {
		        ...
		        e.printObjMemDB("orain objetua MANAGED egoeran dago, baina pasahitz berria ikusteko, lehenago kommit egin behar da", u);
		        em.getTransaction().commit();
		    ...
		    u.setPasahitza("1237");
		    e.printObjMemDB("u.setPasahitza(\"1237\") ondo exekutatu da, baina objektua ez dago konektatuta datu-basearekin (detached)", u);

		    em = JPAUtil.getEntityManager();
		    try {
		        em.getTransaction().begin();
		        u = em.find(Erabiltzailea.class, u.getIzena());
		        System.out.println("find => MANAGED");
		        e.printObjMemDB("Berriz errekuperatu da erabiltzailea datu-basetik, baina pasahitzaren aldaketa galdu egin da", u);
		        em.remove(u);
		    ...
		    // CREAR UN NUEVO EntityManager PARA LA ÚLTIMA OPERACIÓN
		    EntityManager finalEm = JPAUtil.getEntityManager();
		    try {
		        finalEm.getTransaction().begin();
		        List<LoginGertaera> result = finalEm.createQuery("from LoginGertaera", LoginGertaera.class).getResultList();

		        for (LoginGertaera ev : result) {
		            System.out.println("Id: " + ev.getId() + " Deskribapena: " +
		                    ev.getDeskribapena() + " Data: " + ev.getData() + " Login: " +
		                    ev.isLogin());
		        }
		        finalEm.getTransaction().commit();
		    } catch (Exception ex) {
		        if (finalEm.getTransaction().isActive()) {
		            finalEm.getTransaction().rollback();
		        }
		        throw ex;
		    } finally {
		        finalEm.close();
...
/*
	...
	private void createAndStoreLoginGertaera(String erabil,boolean login,Date data) {
		...
		try {
			...
			TypedQuery<Erabiltzailea> q = em.createQuery("SELECT e FROM Erabiltzailea e WHERE e.izena = :erabiltzailea", Erabiltzailea.class);
